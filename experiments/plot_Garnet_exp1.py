...
def main():
    ...
    dr_result_path = result_dir / "Garnet_exp1_domain_randomization.csv"
    ...
    if not dr_result_path.exists():
        raise FileNotFoundError(
            ...
            f"{dr_result_path}. Run experiments/Garnet_exp1_domain_randomization.py first."
        )

    # The original three methods and domain randomization are generated by
    # separate scripts, but use the same row schema and experimental seeds.
    df = pd.concat(
    ...
    perf = perf.dropna(subset=["seed", "iteration", "normalized_performance"])
    perf = perf[perf["iteration"].astype(int) >= 0].copy()

    if perf.empty:
        raise ValueError(
            "No matching performance rows found. "
            "Please make sure the two Garnet Exp1 result files contain "
            "Maximum-based, Similarity-aware, Uniform, and Non-robust DR."
        )
    ...
    print(f"Read domain-randomization results from: {dr_result_path}")
